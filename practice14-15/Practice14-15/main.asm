...
;// 18 May 2014
;// Practice 14-15
;// This program modifies lab 12 to add a call to another ShowParams procedure that contains a macro call and compares the result of the two in the comments
INCLUDE Irvine32.inc		;// Irvine's library that contains WriteString, WriteInt, and ReadInt

INCLUDE Macros.inc ;//Include the books macros, contains mDumpMem
...
ShowParams2 PROTO, paramNum: dword ;//prototype for the ShowParams2 procedure, lists its parameters
...
arrayType:dword, arrayOff: dword, arrayLen:dword ;//parameter list, 3 parameter (using local variables will take care of the stack automatically)
	...
	invoke ShowParams2, 8 ;//call to the ShowParams2 procedure that uses the DumpMem macro
...
arrayType:dword, arrayOff: dword, arrayLen: dword, arraySize:dword ;//parameter list, 4 parameters (using local variables will take care of the stack automatically)
	...
	invoke ShowParams2, 9 ;//call to the ShowParams2 procedure that uses the DumpMem macro
...
ShowParams2 PROC, ;// user defined procedure that will print a number of sitems from the stack using a call to the macro mDumpMem
	paramNum: dword ;//parameter list, 1 parameter (using local variables will take care of the stack automatically)
		  mDumpMem ebp,paramNum,type dword ;//this dumps the same thing as the ShowParams procedure except with one different item on the stack, 
										   ;//this item is different because the return addressess are different, if for some reason the two pieces
										   ;//of data were the same then there would be an error because this function would be returning to the wrong place
										   ;//also note that the return address that is shown comes from the call to ShowParams2 and not the macro because
										   ;//the macro just inserts code to the corresponding macro call
	ret ;//returns to where it was called
ShowParams2 ENDP ;//ends the ShowParams procedure
